...
import { Taluka } from '@prisma/client'
import { useTaluka } from '../talukas/taluka-context'
import { DeleteTaluka } from '../talukas/actions'
import TalukaUpdate from './taluka-update'
...
function TalukaTable() {
  const [dataToEdit, setDataToEdit] = useState<Taluka>({
    taluka_id: "",
    name: "",
    district_id: '',
    area_code: "",
  ...
  const { loading, fetchData, search, data, pageSize, sortField, setSortFiled, sortOrder, setSortOrder, setPageSize, currentPage, totalData, setCurrentPage, refresh, refreshKey } = useTaluka()
  useEffect(() => {
    const where = [];
    if (sdm) {
      where.push({ key: 'subdivision_id', value: sdm.subdivision_id })
    }
    if (district) {
      where.push({ key: 'district_id', value: district.district_id })
    }
    fetchData({ pageSize, search, currentPage, sortField, sortOrder: sortOrder == 'asc' ? "asc" : "desc", where });
    //eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pageSize, sortOrder, sortField, currentPage, search, refreshKey, sdm, district])


  const handleDeleteItem = async () => {
    const loading = toast.loading('Deleting...')
    const res = await DeleteTaluka({ id: idToDelete })
    if (res.success) {
  ...
  return (
    <>
      ...
      >
        <TalukaUpdate data={dataToEdit} setOpen={setOpenUpdate} />
      </SharedCreateDialog>
